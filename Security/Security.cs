...
namespace Security
{
    public static class Security
    {
                    ...
                    using (ICryptoTransform encryptor = symmetricKey.CreateEncryptor(keyBytes, ivStringBytes))
                    {
                        using (MemoryStream memoryStream = new MemoryStream())
                        {
                            using (CryptoStream cryptoStream = new CryptoStream(memoryStream, encryptor, CryptoStreamMode.Write))
                            {
                                ...
                                //write encrypted data to file
                                using (FileStream fileStream = new FileStream("DATA.enc", FileMode.Create))
                                {
                                    fileStream.Write(cipherTextBytes, 0, cipherTextBytes.Length);
                                }
                            }
        ...
        /// <param name="filePath"></param>
        /// <param name="passPhrase"></param>
        public static bool DecryptFile(string filePath, string passPhrase, string outputPath)
        {
            try
            {
                // Get the complete stream of bytes that represent:
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("Failed to decrypt: {0}", ex.Message);
                return false;
            }
        }
